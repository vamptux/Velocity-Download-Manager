...
use super::runtime_transfer::{
    run_segment_worker, run_unknown_size_stream_worker, InitialResponseStream,
...
impl RuntimeWakeLockGuard {
    fn acquire(engine: &EngineState) -> Self {
...
fn take_disk_pool_error(disk_pool: &disk_pool::DiskPool) -> Result<(), String> {
...
async fn wait_for_disk_pool_drain(disk_pool: &disk_pool::DiskPool) -> Result<(), String> {
...
fn classify_unknown_size_failure_kind(error: &str) -> DownloadFailureKind {
...
async fn run_unknown_size_stream_runtime(
    engine: &EngineState,
    runtime_download: &mut DownloadRecord,
    http_pool: &http_pool::HttpPool,
