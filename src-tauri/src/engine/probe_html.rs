...
mod tests {
    ...
    use super::{
        extract_html_app_api_hint, extract_html_direct_download_url, extract_html_filename_hint,
        extract_html_follow_up_request, generate_html_app_website_token_for_bucket,
        is_html_interstitial_response, parse_html_app_api_config, select_html_app_direct_download,
        HtmlAppContentChild, HtmlAppContentData, HtmlFollowUpMethod,
    };
    ...
    #[test]
    fn extracts_app_api_hint_from_wrapper_shell() {
        let html = r#"
                        <html>
                            <head>
        ...
        let hint = extract_html_app_api_hint(
            "https://gofile.io/d/87510386-18a2-4956-9183-e657ce6390b5",
            html,
        )
        .expect("expected app-api hint");

        assert_eq!(hint.content_id, "87510386-18a2-4956-9183-e657ce6390b5");
        assert_eq!(
            hint.config_url.as_deref(),
            Some("https://gofile.io/dist/js/config.js")
        );
    }

    #[test]
    fn extracts_app_api_hint_from_inline_config_blob() {
        let html = r#"
                        <html>
                            <head>
                                <script>
                                    window.__APP_CONFIG__ = {
                                        "apiBaseUrl": "https://api.gofile.io",
                                        "websiteTokenSeed": "inline-seed-42"
                                    };
                                </script>
                            </head>
                        </html>
        ...
        .expect("expected inline app-api hint");

        assert_eq!(
            hint.inline_api_base_url.as_deref(),
            Some("https://api.gofile.io")
        );
        assert_eq!(
            hint.inline_website_token_seed.as_deref(),
            Some("inline-seed-42")
        );
    }

    #[test]
    fn parses_app_api_config_values_from_script() {
        let config = r#"
                        appdata.wt = "4fd6sg89d7s6";
                        appdata.apiServer = "api";
                "#;

        let parsed = parse_html_app_api_config("https://gofile.io/dist/js/config.js", config)
            .expect("expected parsed app-api config");

        assert_eq!(parsed.api_base_url, "https://api.gofile.io");
        assert_eq!(parsed.website_token_seed, "4fd6sg89d7s6");
    }

    #[test]
    fn extracts_post_follow_up_request_from_wrapper_form() {
        let html = r#"
                        <html>
                            <body>
                                <form method="post" action="/download">
                                    ...
                                    <input type="hidden" name="rand" value="tkn-42" />
                                </form>
                            </body>
                        </html>
                "#;

        let request = extract_html_follow_up_request("https://example.com/share/abc123", html)
            .expect("expected follow-up request");

        assert_eq!(request.url, "https://example.com/download");
        ...
        assert_eq!(
            ...
            vec![
                ...
                ("rand".to_string(), "tkn-42".to_string()),
            ]
        );
    }

    #[test]
    fn generates_expected_app_website_token_hash() {
        assert_eq!(
            generate_html_app_website_token_for_bucket(
            ...
            "4966d06de163ec041ca18ee88ffbbdc5acb13a8eccaa719744339408bb185629"
        );
    }

    #[test]
    fn selects_single_child_direct_download_from_app_content() {
        let content = HtmlAppContentData {
            kind: Some("folder".to_string()),
            children: BTreeMap::from([(
                "child-1".to_string(),
                HtmlAppContentChild {
                    kind: Some("file".to_string()),
                    name: Some("toolkit.zip".to_string()),
                    link: Some("https://cdn.example.com/releases/toolkit.zip".to_string()),
                },
            )]),
            ..HtmlAppContentData::default()
        };

        let resolved = select_html_app_direct_download(&content, "accountToken=test123")
            .expect("expected resolved direct download");
        assert_eq!(
            resolved.direct_download_url,
            "https://cdn.example.com/releases/toolkit.zip"
        );
        assert_eq!(resolved.suggested_name.as_deref(), Some("toolkit.zip"));
    }
}
