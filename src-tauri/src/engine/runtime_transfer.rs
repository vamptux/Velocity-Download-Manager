...
mod tests {
    ...
    use reqwest::{
        header::{HeaderMap, HeaderValue, CONTENT_LENGTH, CONTENT_RANGE},
        StatusCode,
    };
    ...
    #[test]
    fn trims_buffer_when_dynamic_end_shrinks_mid_flush() {
        assert_eq!(
            plan_buffered_write(100, 149, 60),
            BufferedWritePlan {
                bytes_to_write: 50,
                boundary_reached: true,
            }
    ...
    #[test]
    fn keeps_full_buffer_when_boundary_is_not_reached() {
        assert_eq!(
            plan_buffered_write(100, 199, 60),
            BufferedWritePlan {
                bytes_to_write: 60,
                boundary_reached: false,
            }
    ...
    #[test]
    fn rejects_http_200_without_matching_range() {
        let mut headers = HeaderMap::new();
        headers.insert(CONTENT_LENGTH, HeaderValue::from_static("900"));

        let error = validate_range_response(
            StatusCode::OK,
            &headers,
            FetchSegmentRequest {
                start: 100,
        ...
        assert!(error.contains("ignored the requested byte range"));
    }

    #[test]
    fn accepts_full_first_range_when_length_matches() {
        let mut headers = HeaderMap::new();
    ...
    #[tokio::test]
    async fn zero_rate_limit_bypasses_waiting() {
