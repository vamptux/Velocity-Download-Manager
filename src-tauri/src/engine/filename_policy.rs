...
#[cfg(test)]
mod tests {
    use super::{guard_windows_reserved, sanitize_filename_leaf};

    #[test]
    fn collapses_multiple_spaces() {
        assert_eq!(
            sanitize_filename_leaf("my   file   name.rar"),
            Some("my file name.rar".to_string())
        );
    }

    #[test]
    fn strips_invisible_unicode() {
        let name = "file\u{200B}name\u{FEFF}.zip";
        assert_eq!(
            sanitize_filename_leaf(name),
    ...
    fn windows_reserved_names_are_prefixed() {
        assert_eq!(guard_windows_reserved("CON".to_string()), "_CON");
        assert_eq!(guard_windows_reserved("con.txt".to_string()), "_con.txt");
        assert_eq!(guard_windows_reserved("NUL".to_string()), "_NUL");
        assert_eq!(guard_windows_reserved("COM1.dat".to_string()), "_COM1.dat");
        assert_eq!(
            guard_windows_reserved("normal.txt".to_string()),
            "normal.txt"
        );
    }

    #[test]
    fn sanitizes_windows_unsafe_filenames() {
        assert_eq!(
            sanitize_filename_leaf("..\\CON?.zip").as_deref(),
            Some("CON_.zip")
        );
        assert_eq!(
            sanitize_filename_leaf("CON.zip").as_deref(),
    ...
    fn long_filename_is_truncated_preserving_extension() {
        ...
        let name = format!("{long_stem}.zip");
        let result = sanitize_filename_leaf(&name).unwrap();
