...
#[cfg(test)]
mod tests {
    use super::super::download_identity::suggested_name_from_url;
    use super::{extract_filename_from_content_disposition, resolve_suggested_name};

    #[test]
    fn parses_rfc5987_content_disposition_filename() {
        let value = "attachment; filename*=UTF-8''Fedora%20Workstation.iso";
        assert_eq!(
            extract_filename_from_content_disposition(Some(value)).as_deref(),
            Some("Fedora Workstation.iso")
        );
    }

    #[test]
    fn falls_back_to_query_filename_when_url_path_is_generic() {
        let (name, compatibility, warnings) = resolve_suggested_name(
            "https://example.com/download?file=installer.exe",
            "https://cdn.example.com/get.php?file=installer.exe",
            None,
        ...
        assert_eq!(
            compatibility.filename_source.as_deref(),
    ...
    #[test]
    fn decodes_percent_encoded_path_filename_candidates() {
        let (name, compatibility, warnings) = resolve_suggested_name(
            "https://example.com/download?id=42",
            "https://cdn.example.com/files/Rust%20v262%20Build%2003052026.part01.rar",
            None,
            Some("application/vnd.rar"),
            None,
        );

        assert_eq!(name, "Rust v262 Build 03052026.part01.rar");
        assert_eq!(compatibility.filename_source.as_deref(), Some("final-url"));
        assert!(warnings.is_empty());
    }

    #[test]
    fn infers_extension_from_mime_when_name_lacks_one() {
        let (name, compatibility, warnings) = resolve_suggested_name(
            "https://example.com/download",
            "https://example.com/download",
            Some("attachment; filename=release"),
        ...
        assert_eq!(
            compatibility.classification.as_deref(),
            Some("mime-extended")
        );
    ...
    #[test]
    fn shared_policy_matches_identity_suggestions() {
        let url = "https://example.com/files/file%E2%80%8Bname.zip";
        let (resolved, _, warnings) =
            resolve_suggested_name(url, url, None, Some("application/zip"), None);

        assert_eq!(resolved, suggested_name_from_url(url));
        assert!(warnings.is_empty());
    }

    #[test]
    fn prefers_html_wrapper_filename_hint_over_generic_route() {
        let (name, compatibility, warnings) = resolve_suggested_name(
            "https://store-eu-par-1.gofile.io/download/web/ddjioeqwhdu9w3",
            "https://store-eu-par-1.gofile.io/download/web/ddjioeqwhdu9w3",
            None,
            Some("text/html"),
        ...
        assert_eq!(name, "Human_Fall_Flat_Build_01152026.rar");
        assert_eq!(
            compatibility.filename_source.as_deref(),
            Some("html-wrapper")
        );
    ...
    #[test]
    fn prefers_original_filename_path_over_redirected_share_code() {
        let (name, compatibility, warnings) = resolve_suggested_name(
            "https://store-eu-par-1.gofile.io/download/web/ba9ee292-11f8-43a6-88f0-fa409fc4f796/Human%20Fall%20Flat%20Build%2001152026.rar",
        ...
        assert_eq!(name, "Human Fall Flat Build 01152026.rar");
        assert_eq!(
            compatibility.filename_source.as_deref(),
            Some("original-url")
        );
