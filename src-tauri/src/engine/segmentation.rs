...
#[cfg(test)]
mod tests {
    use super::{compute_segments_with_hints, SegmentPlanningHints};

    #[test]
    fn falls_back_to_equal_segments_without_throughput_hints() {
        let hinted = compute_segments_with_hints(
            1_000,
            4,
            100,
            4,
            SegmentPlanningHints::default(),
        );

        assert_eq!(hinted.len(), 4);
        assert_eq!(hinted[0].start, 0);
        assert_eq!(hinted[0].end, 249);
        assert_eq!(hinted[3].start, 750);
        assert_eq!(hinted[3].end, 999);
    }

    #[test]
    fn throughput_shaped_plan_biases_early_segments_smaller() {
        let segments = compute_segments_with_hints(
            10_000,
            4,
            500,
            4,
            SegmentPlanningHints {
                throughput_bytes_per_second: Some(2_000),
                ttfb_ms: Some(0),
        ...
        let sizes: Vec<u64> = segments
            .iter()
            .map(|segment| (segment.end - segment.start + 1).max(0) as u64)
            .collect();
    ...
    #[test]
    fn high_ttfb_expands_initial_window_to_reduce_early_churn() {
        let low_ttfb = compute_segments_with_hints(
            10_000,
            4,
            500,
            4,
        ...
        let high_ttfb = compute_segments_with_hints(
            10_000,
            4,
            500,
            4,
        ...
        let low_first = (low_ttfb[0].end - low_ttfb[0].start + 1).max(0) as u64;
        let high_first = (high_ttfb[0].end - high_ttfb[0].start + 1).max(0) as u64;
