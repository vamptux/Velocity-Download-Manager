...
mod tests {
    ...
    use super::finalize_download_paths;
    ...
    fn cross_volume_error() -> io::Error {
        io::Error::from_raw_os_error(CROSS_VOLUME_OS_ERROR)
    }

    #[test]
    fn restores_previous_target_when_final_move_fails() {
        let sandbox = TestSandbox::new();
        let temp = sandbox.path("file.bin.part");
        let target = sandbox.path("file.bin");
        let backup = sandbox.path("file.bin.vdm-replaced-0");

        fs::write(&temp, b"new payload").unwrap();
        fs::write(&target, b"old payload").unwrap();

        let temp_for_rename = temp.clone();
        let target_for_rename = target.clone();
        let mut rename_file = move |from: &Path, to: &Path| {
            if from == temp_for_rename.as_path() && to == target_for_rename.as_path() {
                Err(io::Error::new(
        ...
        let error = match finalize_download_paths(&temp, &target, &mut rename_file) {
            Ok(_) => panic!("expected final move failure"),
            Err(error) => error,
        };

        assert!(error.contains("Failed moving temp file into target location"));
    ...
    #[test]
    fn signals_copy_fallback_on_cross_volume_move() {
        let sandbox = TestSandbox::new();
        let temp = sandbox.path("file.bin.part");
        let target = sandbox.path("file.bin");
        let backup = sandbox.path("file.bin.vdm-replaced-0");
        ...
        let mut rename_file = move |from: &Path, to: &Path| {
            if from == temp_for_rename.as_path() && to == target_for_rename.as_path() {
                Err(cross_volume_error())
            } else {
                fs::rename(from, to)
            }
        };

        let result = finalize_download_paths(&temp, &target, &mut rename_file).unwrap();

        assert!(result.used_copy_fallback);
